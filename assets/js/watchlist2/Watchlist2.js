async function Watchlist2() {
  try {
    let data = await fetch("http://localhost:3000/watchList2");
    let res = await data.json();
    let watchList2 = res.map(function (elem) {
      return `
      <div class="swiper-slide">
              <!-- Start img part -->
              <div class="mommy">
                <img src="${elem.picture}" alt="${elem.Alter}" />
              ...
              <!-- End descriotion part -->
            </div>
        `;
    });
    document
      .querySelector(".watch2wrapper")
      .insertAdjacentHTML("afterbegin", watchList2.join());
  } catch (error) {
